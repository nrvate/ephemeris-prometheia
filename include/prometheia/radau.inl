    ...
    static const Tables t = [] {
        ...
        for (int col = 0; col < kM; ++col) {
            ...
            for (int r = col + 1; r < kM; ++r) {
                if (std::fabs(aug[r][col]) > std::fabs(aug[piv][col]))
                    piv = r;
            }
            std::swap(aug[col], aug[piv]);
            const double d = aug[col][col];
            for (double& x : aug[col])
                x /= d;
            for (int r = 0; r < kM; ++r) {
                if (r == col)
                    continue;
                ...
                if (f == 0.0)
                    continue;
                for (int c = 0; c < 2 * kM; ++c)
                    aug[r][c] -= f * aug[col][c];
            }
        }
        for (int i = 0; i < kM; ++i) {
            for (int j = 0; j < kM; ++j)
                tb.vinv[i * kM + j] = aug[i][kM + j];
        }
        return tb;
    }();
    return t;
...
Result<void> integrate_radau15(double* y, double t0, double t1, Force&& accel,
                               const Radau15Options& opts, Radau15Stats* stats, Sink&& sink) {
    if (t1 == t0)
        return {};
    ...
    while ((t1 - t) * direction > 0.0) {
        ...
        const double f_rescale = h_last != 0.0 ? h / h_last : 0.0;
        double fscale = 1.0;
        for (int j = 0; j < 8; ++j) {
            for (int c = 0; c < 3; ++c) {
        ...
        for (int iter = 0; iter < opts.max_iterations && !converged; ++iter) {
            ...
            for (int i = 0; i < kM; ++i) {
                for (int c = 0; c < 3; ++c)
                    den = std::max(den, std::fabs(a[i][c]));
            }
        ...
        const bool accept = ratio <= eps_b || std::fabs(h) <= 1e-14 * std::fabs(span);
        double factor;
        if (accept) {
            ...
            factor = std::clamp(std::pow(eps_b / std::max(ratio, 1e-300), 0.142857), 0.2, 5.0);
        } else {
