...
class WindowMemo {
 ...
 private:
  ...
  State eval_from_samples(double t) const {
    ...
    if (lo + 1 >= samples_.size()) return sample_to_state(samples_.back());
    const TrajSample& a = samples_[lo];
    const TrajSample& b = samples_[lo + 1];
    const double dt = b.t - a.t;
    if (dt == 0.0) return sample_to_state(a);
  ...
  bool integrate_window(double from, double to, bool forward) {
    ...
    if (forward) {
      ...
      for (size_t i = 1; i < fresh.size(); ++i) samples_.push_back(fresh[i]);
      hi_ = to;
    } else {
      ...
      for (size_t i = fresh.size(); i-- > 1;) merged.push_back(fresh[i]);
      for (const TrajSample& p : samples_) merged.push_back(p);
