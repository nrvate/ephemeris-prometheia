...
| `ephproto/v4/` | Astrolog's ephemeris protocol **version 4** (`ephproto.h`, the byte-level authority prometheiad speaks) + its generated `registries.json` | Astrolog `ephv4` commit `eed6429` (header and registries; unchanged at `472b21a`) | GPL-2.0 (Astrolog, `license.htm` in its tree) | the per-kind correction mask drop (WELCOME tag 0x0014, drop text revision 3), verified at `472b21a`: set-sha256 `d904e3586c25dc0eeedb0cebb316be92f04b086c2e4d96ae4d2e3bf92337c72d` over the 99 conformance fixtures, 99/99, plus `JUDGEMENTS.tsv` (set-sha256 `f1a36d5b…`, over its non-comment lines as written), 7/7 (`tools/check/ephproto4_fixtures.py`, verdicts agreed with Astrolog 2026-09-18). Previously `0fbc863`, the corrApplied drop (set `1c934c7d…`, 91/91). That pin was one line older than the §3 lock commit `cf83dc9`: the lock added `ValidateWelcome`'s `maxPayload = 0` refusal, and the header and registries were otherwise identical (diffed 2026-09-18) |
