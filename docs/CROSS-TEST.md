...
- **A check that has never gone red is not known to work.** Fault-inject before
  ...
  - **And a falsification recorded as prose is a measurement taken once.**
    ...
    `ephsrv-soak.sh --selftest` is theirs. **Two checkers under
    `tools/check/` still carry prose** -- `crossrun.py` and
    `ephproto4_fixtures.py` -- named rather than counted,
    because a number drifts and a list does not; so the hole is narrowed
    by six legs, not closed — which is what their side said of theirs,
