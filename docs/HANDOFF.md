...
- **Landed: the per-kind correction mask drop** (WELCOME tag 0x0014,
  ephv4 `eed6429`, SERVER.md "Per-kind correction masks"). Fixtures 99/99
  and judgements 7/7 from the text first, the vendored pair re-pinned, and no server change.
...
- **Logging, everywhere, for traceability.** Review what each component
  ...
  - Done 2026-09-18, the rest of this side:
    - the cross-test TSV's `req_ours`/`req_theirs` columns name the
      requests behind every row;
    - the fetch tools log one UTC-timestamped line per GET, with its URL,
