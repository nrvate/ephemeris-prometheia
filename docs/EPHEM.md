...
```
# JPL DE440 binary (linux_p1550p2650.440), ephem 0.5.0
...
- `--sidereal fagan-bradley|fb|lahiri|user:JD:DEG|tropical`. For `user`, DEG is the mean ayanamsha at the TT Julian date JD.
  ...
  `galequ-iau1958`, `galequ-true` and `galequ-mula`
  ([FRAMES.md](FRAMES.md), "Zodiacs defined at the instant"). They have no
  anchor epoch, so `--sid-plane anchor` is an error with them.
