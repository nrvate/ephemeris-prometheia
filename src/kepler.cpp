...
double wrap_pi(double x) {
  ...
  x = std::fmod(x, two_pi);
  if (x > 3.1415926535897932384626433832795) x -= two_pi;
  if (x < -3.1415926535897932384626433832795) x += two_pi;
  return x;
}
...
double solve_elliptic_e(double e, double M) {
  ...
  for (int i = 0; i < kNewtonMaxIter; ++i) {
    ...
    E -= dE;
    if (std::fabs(dE) <= kNewtonTol * (1.0 + std::fabs(E))) break;
  }
  return E;
}
...
Result<State> elements_to_state(double mu, const Elements& el) {
  ...
  if (!std::isfinite(el.a) || el.a == 0.0 || !std::isfinite(el.e) || el.e < 0.0 ||
      el.e == 1.0) {
...
Result<Elements> state_to_elements(double mu, const State& s) {
  if (!(mu > 0.0)) return make_error(ErrorCode::ArgumentError, "mu must be positive");
  const double r = norm(s.pos);
  const double v2 = norm2(s.vel);
  if (!(r > 0.0)) return make_error(ErrorCode::ArgumentError, "degenerate state");

  const Vec3 hvec = cross(s.pos, s.vel);
  const double h = norm(hvec);
  if (!(h > 0.0)) return make_error(ErrorCode::ArgumentError, "radial orbit: h = 0");
  ...
  if (el.inc < 1e-12) nvec = Vec3(1.0, 0.0, 0.0);      // equatorial: node undefined
  const double cos_argp = dot(nvec, evec) / (norm(nvec) * norm(evec) + 1e-300);
  double argp = std::acos(std::clamp(cos_argp, -1.0, 1.0));
  if (dot(cross(nvec, evec), hvec) < 0.0) argp = 6.283185307179586476925286766559 - argp;
  ...
  if (dot(s.pos, s.vel) < 0.0) nu = 6.283185307179586476925286766559 - nu;
  if (el.e < 1.0) {
...
Result<State> kepler_propagate(double mu, const State& s0, double t0, double t1) {
  ...
  if (!el.ok()) return Result<State>(el.error());
