...
void put_head(std::string& out, uint8_t major, uint64_t value) {
  ...
  for (int i = nbytes - 1; i >= 0; --i) out.push_back(char((value >> (8 * i)) & 0xFF));
}
...
Result<double> decode_double_from_head(uint8_t byte, const char* p, const char* end,
  ...
  if (info == 25) {  // half precision
    if (end - p < 2) return make_error(ErrorCode::FormatError, "CBOR: truncated half float");
  ...
  if (info == 26) {  // single precision
    if (end - p < 4) return make_error(ErrorCode::FormatError, "CBOR: truncated float");
    uint32_t bits = 0;
    for (int k = 0; k < 4; ++k) bits = (bits << 8) | uint8_t(p[k]);
    float f;
    std::memcpy(&f, &bits, 4);
    consumed = 5;
    return double(f);
  ...
  if (info == 27) {  // double precision
    if (end - p < 8) return make_error(ErrorCode::FormatError, "CBOR: truncated double");
    uint64_t bits = 0;
    for (int k = 0; k < 8; ++k) bits = (bits << 8) | uint8_t(p[k]);
    double d;
    std::memcpy(&d, &bits, 8);
    consumed = 9;
    return d;
...
Result<CborValue> decode_one(DecodeState& st) {
  if (st.p == st.end) return make_error(ErrorCode::FormatError, "CBOR: unexpected end");
  if (++st.depth > kMaxDepth) return make_error(ErrorCode::FormatError, "CBOR: nesting too deep");
  ...
  auto read_arg = [&](uint64_t& arg) -> bool {
    ...
    if (nbytes < 0 || st.end - st.p < nbytes) return false;
    arg = 0;
    for (int k = 0; k < nbytes; ++k) arg = (arg << 8) | uint8_t(*st.p++);
    return true;
  };
  ...
  switch (major) {
    case kMajorUnsigned: {
      uint64_t arg;
      if (!read_arg(arg)) return make_error(ErrorCode::FormatError, "CBOR: truncated integer");
    ...
    case 2: {  // byte string — surfaced as Text with raw bytes (metadata only)
      uint64_t arg;
      if (!read_arg(arg)) return make_error(ErrorCode::FormatError, "CBOR: truncated bytes");
    ...
    case kMajorText: {
      uint64_t arg;
      if (!read_arg(arg)) return make_error(ErrorCode::FormatError, "CBOR: truncated text");
      if (uint64_t(st.end - st.p) < arg)
    ...
    case kMajorArray: {
      uint64_t arg;
      if (!read_arg(arg)) return make_error(ErrorCode::FormatError, "CBOR: truncated array");
    ...
    case kMajorMap: {
      uint64_t arg;
      if (!read_arg(arg)) return make_error(ErrorCode::FormatError, "CBOR: truncated map");
...
const CborValue* CborValue::find(std::string_view key) const {
  ...
  for (size_t k = 0; k + 1 < items.size(); k += 2) {
    if (items[k].type == Type::Text && items[k].text == key) return &items[k + 1];
  }
  return nullptr;
}
...
void cbor_encode(std::string& out, const CborValue& v) {
  switch (v.type) {
    ...
    case CborValue::Type::Double: {
      ...
      for (int k = 7; k >= 0; --k) out.push_back(char((bits >> (8 * k)) & 0xFF));
      break;
    }
    case CborValue::Type::Array:
      put_head(out, kMajorArray, v.items.size());
      for (const auto& item : v.items) cbor_encode(out, item);
      break;
    case CborValue::Type::Map:
