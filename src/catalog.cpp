...
void append_chunk_index_entry(std::string& out, uint64_t data_offset,
                              uint64_t stored_size, uint64_t raw_size,
                              uint32_t crc, uint32_t record_count,
...
void encode_record(std::string& out, const Record& r) {
  ...
  if (r.has(RecordFlags::kSigmas)) {
    for (double s : r.sigmas) put_f32(out, float(s));
  }
  if (r.has(RecordFlags::kHg)) {
    put_f32(out, r.h_mag);
    put_f32(out, r.g_slope);
  }
  if (r.has(RecordFlags::kDiameter)) put_f32(out, r.diameter_km);
  put_uvarint(out, r.name_offset);
}

// Decodes one record.
const char* decode_record(const char* p, const char* end, std::string_view pool,
                          Record& r, Error& err) {
  ...
  uint64_t spkid;
  if (!(p = get_uvarint(p, end, spkid))) return fail("truncated spkid");
  if (end - p < 2) return fail("truncated class/flags");
  const uint8_t body_class = uint8_t(p[0]);
  const uint8_t flags = uint8_t(p[1]);
  p += 2;
  if (!body_class_valid(body_class)) return fail("unknown body class");
  if (flags & ~kKnownFlagMask) return fail("unknown record flags");

  r = Record{};
  r.spkid = spkid;
  r.body_class = BodyClass(body_class);
  r.flags = flags;
  if (!(p = get_f64(p, end, r.epoch_jtdb))) return fail("truncated epoch");
  if (!(p = get_f64(p, end, r.a_au))) return fail("truncated elements");
  if (!(p = get_f64(p, end, r.e))) return fail("truncated elements");
  if (!(p = get_f64(p, end, r.inc_rad))) return fail("truncated elements");
  if (!(p = get_f64(p, end, r.node_rad))) return fail("truncated elements");
  if (!(p = get_f64(p, end, r.argp_rad))) return fail("truncated elements");
  if (!(p = get_f64(p, end, r.mean_anom_rad))) return fail("truncated elements");
  if (flags & RecordFlags::kSigmas) {
    for (double& s : r.sigmas) {
      float f32v;
      if (!(p = get_f32(p, end, f32v))) return fail("truncated sigmas");
      s = f32v;
    }
  }
  if (flags & RecordFlags::kHg) {
    if (!(p = get_f32(p, end, r.h_mag))) return fail("truncated H");
    if (!(p = get_f32(p, end, r.g_slope))) return fail("truncated G");
  }
  if (flags & RecordFlags::kDiameter) {
    if (!(p = get_f32(p, end, r.diameter_km))) return fail("truncated diameter");
  }
  if (!(p = get_uvarint(p, end, r.name_offset))) return fail("truncated name offset");
  if (r.name_offset > pool.size()) return fail("name offset out of pool range");
  return p;
}
...
Result<void> Writer::flush_chunk() {
  ...
  entries_.push_back(ChunkEntry{file_pos_, stored.size(), raw_size, first_spkid,
                                crc, uint32_t(chunk_count_in_buf_)});
...
Result<void> Writer::add(const Record& r, std::string_view pdes,
                         std::string_view name) {
  if (f_ == nullptr) return make_error(ErrorCode::ArgumentError, "writer is closed");
  if (pdes.empty()) return make_error(ErrorCode::ArgumentError, "pdes must be non-empty");
  if (have_last_ && r.spkid <= last_spkid_) {
    return make_error(ErrorCode::ArgumentError,
                      "spkid " + std::to_string(r.spkid) +
  ...
  if (!finite) return make_error(ErrorCode::ArgumentError, "non-finite element");
  if (r.e < 0 || r.e == 1.0) {
  ...
  if (pdes.find('\0') != std::string_view::npos ||
  ...
  if (chunk_count_in_buf_ == 0) first_spkid_of_chunk_ = out.spkid;
  encode_record(chunk_buf_, out);
  ++chunk_count_in_buf_;
  ++count_;
  last_spkid_ = out.spkid;
  have_last_ = true;
  if (uint32_t(chunk_count_in_buf_) >= opts_.chunk_records) return flush_chunk();
  return {};
}
...
Result<void> Writer::finish(CborValue metadata) {
  ...
  if (auto e = flush_chunk(); !e.ok()) return e;

  const uint64_t pool_offset = file_pos_;
  if (!pool_.empty() &&
      std::fwrite(pool_.data(), 1, pool_.size(), f_) != pool_.size()) {
  ...
  const uint64_t index_offset = file_pos_;
  {
    ...
    for (const auto& e : entries_) {
      append_chunk_index_entry(index, e.data_offset, e.stored_size, e.raw_size,
                               e.crc, e.record_count, e.first_spkid);
    }
    if (!index.empty() &&
...
Result<Reader> Reader::open(const std::string& path) {
  ...
  std::string header;
  if (auto e = read_at(0, kHeaderSize, header); !e.ok()) return Result<Reader>(e.error());
  ...
  if (get_u32(footer.data() + 40) != kMagic ||
      get_u16(footer.data() + 44) != kFormatMajor)
    return make_error(ErrorCode::CorruptionError, "footer invalid (truncated file?)");
  if (get_u64(footer.data()) != index_offset ||
      get_u64(footer.data() + 8) != meta_offset)
  ...
  const uint64_t chunk_count =
      r.record_count_ == 0
          ? 0
          : (r.record_count_ + r.chunk_records_ - 1) / r.chunk_records_;
  ...
  if (!meta) return Result<Reader>(std::move(meta.error()));
  ...
  if (chunk_count > 0) {
    ...
    if (total_records != r.record_count_) {
      return make_error(ErrorCode::CorruptionError, "chunk record counts disagree with header");
    }
  }
...
Result<const std::string*> Reader::load_chunk(uint64_t chunk_index) const {
  ...
  if (compressed_) {
    ...
    const size_t rc =
        ZSTD_decompress(raw.data(), raw.size(), stored.data(), stored.size());
...
Result<Record> Reader::lookup(uint64_t spkid) const {
  ...
  if (!chunk.ok()) return Result<Record>(chunk.error());
  ...
  for (uint32_t i = 0; i < entries_[size_t(lo)].record_count; ++i) {
    ...
    if (next == nullptr) return Result<Record>(err);
    if (r.spkid == spkid) return r;
    if (r.spkid > spkid) break;  // sorted: cannot appear later
    p = next;
  }
  return make_error(ErrorCode::NotFound, "spkid not in catalog");
}

Result<void> Reader::for_each(
    const std::function<void(const Record&, const Names&)>& fn) const {
