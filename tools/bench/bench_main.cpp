...
int main(int argc, char** argv) {
  const std::string path =
      argc > 1 ? argv[1] : "tests/data/sample-100.epm";
  auto cat = catalog::Reader::open(path);
  if (!cat.ok()) {
    std::fprintf(stderr, "open %s: %s\n", path.c_str(),
                 cat.error().message.c_str());
    return 1;
  }
  ...
  static Trajectory jup_traj = Trajectory::sample_uniform(
                                   [&](double t) {
                                     return kepler_state(jup, t + epoch, epoch);
                                   },
                                   t0, t1, 2048)
                                   .value();
  static Trajectory sat_traj = Trajectory::sample_uniform(
                                   [&](double t) {
                                     return kepler_state(sat, t + epoch, epoch);
                                   },
                                   t0, t1, 2048)
                                   .value();
  static std::vector<Perturber> perturbers = {
      Perturber{kMuJup, &jup_traj}, Perturber{kMuSat, &sat_traj}};
  ...
  const size_t nA = std::min<size_t>(79, n_bodies);
  {
    ...
    for (size_t i = 0; i < nA; ++i) {
      ...
      IntegrateStats st;
      auto e = integrate_dp54(y, -365.25, 365.25, force,
                              IntegrateOptions{}, &st);
      if (!e.ok()) {
  ...
  // ---- Scenario B: warm memoized evaluations ----------------------------
  {
    ...
    // Prime one window.
    for (int i = 0; i <= 16; ++i) memo.at(365.25 * double(i) / 16.0);
    const auto begin = std::chrono::steady_clock::now();
