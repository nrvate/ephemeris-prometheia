...
const char* class_name(prometheia::catalog::BodyClass c) {
  switch (c) {
    case prometheia::catalog::BodyClass::Asteroid: return "asteroid";
    case prometheia::catalog::BodyClass::Comet: return "comet";
    case prometheia::catalog::BodyClass::Planet: return "planet";
...
int main(int argc, char** argv) {
  ...
  const auto emit_sample = [&](const prometheia::catalog::Record& rec,
    ...
    if (sampled >= uint64_t(sample < 0 ? 0 : sample)) return;
    if (sampled == 0) std::printf("sample records:\n");
    ++sampled;
    std::printf("  spkid %llu  pdes %-16s name %-20s class %-8s epoch %.1f\n",
                (unsigned long long)rec.spkid, std::string(names.pdes).c_str(),
                std::string(names.name).c_str(), class_name(rec.body_class),
                rec.epoch_jtdb);
    std::printf("    a=%.8f AU  e=%.8f  i=%.6f rad  node=%.6f  argp=%.6f  M=%.6f\n",
                rec.a_au, rec.e, rec.inc_rad, rec.node_rad, rec.argp_rad,
                rec.mean_anom_rad);
  ...
  if (auto e = reader.for_each([&](const prometheia::catalog::Record& rec,
                                   const prometheia::catalog::Names& names) {
        const int ci = int(rec.body_class) < 3 ? int(rec.body_class) : 3;
        ++counts[ci];
        if (rec.has(prometheia::catalog::RecordFlags::kSigmas)) ++with_sigma;
        if (rec.has(prometheia::catalog::RecordFlags::kHg)) ++with_hg;
        if (rec.has(prometheia::catalog::RecordFlags::kDiameter)) ++with_diameter;
        if (rec.has(prometheia::catalog::RecordFlags::kHasName)) ++with_name;
