...
struct FieldMap {
  ...
  static std::optional<FieldMap> parse(const std::string& header_line) {
    ...
    if (header_line.rfind(prefix, 0) != 0) return std::nullopt;
    FieldMap fm;
    size_t pos = prefix.size();
    while (pos <= header_line.size()) {
      size_t next = header_line.find('\t', pos);
      if (next == std::string::npos) next = header_line.size();
...
ParseOutcome parse_row(const std::vector<std::string>& cols, const FieldMap& fm,
                       uint64_t line_no) {
  ParseOutcome out;
  ...
  if (end == spkid_s.c_str() || spkid == 0) {
    std::fprintf(stderr, "skipping line %llu: bad spkid '%s'\n",
                 (unsigned long long)line_no, spkid_s.c_str());
    out.skipped = true;
    return out;
  }
  ...
  for (int k = 0; k < 6; ++k) {
    ...
    if (!s.empty()) {
      ...
      if (r.sigmas[k] < 0) r.sigmas[k] = 0.0;  // treat missing as zero, not negative
      have_sigma = true;
    } else {
      r.sigmas[k] = 0.0;
    }
  }
  if (have_sigma) r.flags |= catalog::RecordFlags::kSigmas;
...
int main(int argc, char** argv) {
  ...
  for (const auto& shard : shards) {
    ...
    if (first_shard) {
      ...
      for (const auto& n : fm->names) std::printf(" %s", n.c_str());
      std::printf("\n");
      first_shard = false;
    }
    while (std::getline(in, line)) {
  ...
  std::printf("parsed %llu rows, %llu skipped, %llu kept\n",
              (unsigned long long)lines, (unsigned long long)skipped,
              (unsigned long long)records.size());

  std::vector<uint64_t> order(records.size());
  for (uint64_t i = 0; i < records.size(); ++i) order[i] = i;
  ...
  put_kv("generator", CborValue::make_text(
                          std::string("prometheia-convert ") + prometheia::version_string));
  ...
  put_kv("source", CborValue::make_text(prov.count("source") ? prov.at("source") : "JPL SBDB Query API"));
  ...
  put_kv("sbdb_count",
         CborValue::make_unsigned(std::strtoull(
