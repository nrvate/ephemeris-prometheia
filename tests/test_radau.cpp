...
TEST(radau_circular_10_orbits) {
  ...
  const double period = 6.283185307179586476925286766559 /
                        mean_motion(kMuSun, 1.0);
  Radau15Stats st;
  auto e = integrate_radau15(y, 0.0, 10.0 * period, TwoBody{kMuSun},
                             Radau15Options{}, &st);
  CHECK(e.ok());
  ...
  if (e.ok() && exact.ok()) {
    ...
    std::printf("  circular 10 orbits: dp = %.3e AU (%llu steps, "
                ...
                dp, (unsigned long long)st.steps,
                (unsigned long long)st.accel_evals,
...
TEST(radau_ceres_55yr_vs_dp54) {
  Elements el{2.765552595034094, 0.07969229514816586, 0.1848, 1.4006, 1.2792,
              1.0};
  ...
  Radau15Stats rs;
  auto er = integrate_radau15(yr, 0.0, span, TwoBody{kMuSun},
                             Radau15Options{}, &rs);
  const double radau_ms = std::chrono::duration<double, std::milli>(
  ...
  IntegrateStats ds;
  auto ed = integrate_dp54(yd, 0.0, span, TwoBody{kMuSun}, IntegrateOptions{},
                           &ds);
  const double dp54_ms = std::chrono::duration<double, std::milli>(
  ...
  if (er.ok() && ed.ok() && exact.ok()) {
    ...
    std::printf("  55 yr arc: radau15 dp=%.3e AU (%llu steps, %llu evals, "
                ...
                dpr, (unsigned long long)rs.steps,
                (unsigned long long)rs.accel_evals, radau_ms, dpd,
                (unsigned long long)ds.steps, dp54_ms);
...
TEST(radau_hyperbolic_flyby) {
  ...
  auto e = integrate_radau15(y, 0.0, dt, TwoBody{kMuSun}, Radau15Options{},
                             nullptr);
  CHECK(e.ok());
...
TEST(radau_backward_round_trip) {
  ...
  double y[6] = {s0.value().pos.x, s0.value().pos.y, s0.value().pos.z,
                 s0.value().vel.x, s0.value().vel.y, s0.value().vel.z};
  CHECK(integrate_radau15(y, 0.0, 365.25, TwoBody{kMuSun}, Radau15Options{},
                         nullptr)
            .ok());
  CHECK(integrate_radau15(y, 365.25, 0.0, TwoBody{kMuSun}, Radau15Options{},
                          nullptr)
            .ok());
