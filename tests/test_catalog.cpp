...
std::string temp_path(const char* tag) {
  ...
  return (std::filesystem::temp_directory_path() /
          ("prometheia_test_" + std::string(tag) + "_" + std::to_string(counter++) +
           ".epm"))
      .string();
}
...
void expect_records_equal(const Record& a, const Record& b) {
  ...
  if (a.has(RecordFlags::kSigmas)) {
    for (int i = 0; i < 6; ++i) CHECK(float(a.sigmas[i]) == float(b.sigmas[i]));
  }
...
TEST(roundtrip_small_mixed) {
  ...
  for (uint64_t spkid : {20000001ull, 20000003ull, 20000004ull, 20000010ull}) {
    ...
    CHECK(rec.ok());
    if (rec.ok()) expect_records_equal(rec.value(), expected_for(spkid));
  }
...
TEST(truncation_and_magic) {
  ...
  CHECK(rt.error().code == ErrorCode::CorruptionError ||
        rt.error().code == ErrorCode::FormatError ||
        rt.error().code == ErrorCode::IoError);
...
TEST(uncompressed_variant) {
  ...
  if (r.ok()) {
    ...
    CHECK(rec.ok());
    if (rec.ok()) expect_records_equal(rec.value(), make_asteroid(20000004));
  }
  std::filesystem::remove(path);
}
...
TEST(medium_scale_and_size) {
  ...
  if (r.ok()) {
    ...
    for (uint64_t probe :
         {20000001ull, 20012345ull, 20025000ull, 20049999ull, 20012345ull}) {
      auto rec = reader.lookup(probe);
      CHECK(rec.ok());
      if (rec.ok()) expect_records_equal(rec.value(), expected(probe));
    }
    uint64_t scanned = 0;
